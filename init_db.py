# ==============================================================================
# init_db.py — Criação e população híbrida do banco de dados (SQLite / Postgres)
# ==============================================================================
import os
import sqlite3

# Tenta importar psycopg2 para o ambiente do Render
try:
    import psycopg2
...
if DATABASE_URL and PSYCOPG2_AVAILABLE:
    print("[DB INIT] PostgreSQL (Render) detectado.")
    # O SQLAlchemy/psycopg2 prefere postgresql:// ao invés de postgres://
    ...
    print("[DB INIT] SQLite (Local) detectado.")
    DATABASE_LOCAL = "banco_local.db"
    
    # Diferente do antigo, aqui NÃO apagamos o banco local se ele já existir, 
    # para mantermos a mesma lógica segura do Render.
...
# ==============================================================================
# 2. CRIAÇÃO DAS TABELAS (IF NOT EXISTS)
# ==============================================================================
print("[DB INIT] Verificando e criando tabelas se necessário...")
...
# ==============================================================================
# 3. VERIFICAÇÃO E INSERÇÃO DE DADOS (IDEMPOTÊNCIA)
# ==============================================================================
cur.execute("SELECT COUNT(*) FROM acontecimentos;")
total_eventos = cur.fetchone()[0]

if total_eventos > 0:
    print(f"[DB INIT] O banco já possui {total_eventos} eventos registrados. Pulando a inserção para evitar duplicidade.")
else:
    print("[DB INIT] Banco vazio. Iniciando a inserção dos dados históricos...")
    ...
    """, acontecimentos)
    print(f"[DB INIT] {len(acontecimentos)} acontecimentos inseridos.")
    ...
    cur.executemany(f"""
        ...
        VALUES ({PH}, {PH}, {PH})
    """, personagens)
    print(f"[DB INIT] {len(personagens)} personagens inseridos.")
    ...
    cur.executemany(f"""
        ...
        VALUES ({PH}, {PH}, {PH})
    """, participacoes)
    print(f"[DB INIT] {len(participacoes)} participações N:M inseridas.")

    conn.commit()
    print("[DB INIT] População concluída com sucesso!")
...
cur.close()
conn.close()
print("\n[DB INIT] ✅ Script de inicialização finalizado com sucesso!")